<?php
    ...
    function test_xhr_record_definition_2($properties) {
        ...
        assertEquals($response_array['xhr_record_def_insert_status'], true, 'xhr_record_def_insert_status response status mismatch');
        
        foreach(['t1', 't2', 't3'] as $field_name) {
            assertTrue(isset($response_array['xhr_field_insert_status'][$field_name]), "$field_name not set");
            assertEquals($response_array['xhr_field_insert_status'][$field_name], true, "xhr_record_def_insert_status[$field_name] response status mismatch");
        }
